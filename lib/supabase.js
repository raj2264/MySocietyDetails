...
let SupabaseJs;
try {
  // Prefer explicit CJS entry to avoid Expo Go ESM interop returning default-only modules.
  // eslint-disable-next-line global-require
  SupabaseJs = require('@supabase/supabase-js/dist/index.cjs');
} catch (e) {
  // eslint-disable-next-line global-require
...
// Non-aborting timeout wrapper: prevents UI loaders from waiting forever,
// while avoiding AbortController compatibility issues in Expo Go/Hermes.
const fetchWithTimeout = (input, init = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      if (settled) return;
      settled = true;
      const timeoutError = new Error(`Request timed out after ${SUPABASE_REQUEST_TIMEOUT_MS / 1000}s`);
...
const createClientCompat =
  SupabaseJs?.createClient ||
  SupabaseJs?.default?.createClient ||
  SupabaseJs?.default?.default?.createClient ||
  (typeof SupabaseJs?.default === 'function' ? SupabaseJs.default : null);

let resolvedCreateClient = createClientCompat;

if (typeof resolvedCreateClient !== 'function') {
  try {
    // Extra fallback for environments that expose only ESM entrypoints.
    // eslint-disable-next-line global-require
    const directModule = require('@supabase/supabase-js/dist/index.mjs');
    resolvedCreateClient =
      directModule?.createClient ||
      directModule?.default?.createClient ||
      (typeof directModule?.default === 'function' ? directModule.default : null);
  } catch (e) {
    // Keep original behavior below; this block only broadens compatibility.
  }
}

if (typeof resolvedCreateClient !== 'function') {
  throw new Error(
    `Supabase createClient is unavailable. Export keys: ${Object.keys(SupabaseJs || {}).join(', ')}`
  );
}

export const supabase = resolvedCreateClient(supabaseUrl, supabaseAnonKey, supabaseOptions);
...
// NOTE: initSupabase() is no longer called at module load.
// AuthContext handles session restoration with a cache-first strategy.
// Call initSupabase() explicitly only if needed. 